...
describe("Foreign Keys", () => {
    ...
    it("should not be able to delete category if any app is linked", async done => {
        const categoryId = 6;
        const query = `DELETE FROM categories WHERE id = ${categoryId} AND EXISTS (SELECT 1 FROM linked_apps WHERE category_id = ${categoryId})`;
        try {
    ...
    it("should not be able to delete pricing plan if any app is linked", async done => {
        ...
        const query = `DELETE FROM pricing_plans WHERE id = ${pricingPlanId} AND EXISTS (SELECT 1 FROM linked_apps WHERE pricing_plan_id = ${pricingPlanId})`;
    ...
    it("should not be able to delete app if any data is linked", async done => {
        const appId = 245;
        const query = `DELETE FROM apps WHERE id = ${appId} AND EXISTS (SELECT 1 FROM linked_data WHERE app_id = ${appId})`;
