...
const insertMovieGenres = (
  ...
  const values = genres.map(genre => {
    ...
    if (!genreRow) {
      throw new Error(`Genre '${genre}' not found in the database.`);
    }
...
const insertMovieActors = (
  ...
  const values = actors.map(actor => {
    ...
    if (!actorRow) {
      throw new Error(`Actor '${actor}' not found in the database.`);
    }
...
const insertMovieDirectors = (
  ...
  const values = directors.map(director => {
    ...
    if (!directorRow) {
      throw new Error(`Director '${director}' not found in the database.`);
    }
...
const insertMovieKeywords = (
  ...
  const values = keywords.map(keyword => {
    ...
    if (!keywordRow) {
      throw new Error(`Keyword '${keyword}' not found in the database.`);
    }
...
const insertMovieProductionCompanies = (
  ...
  const values = productionCompanies.map(company => {
    ...
    if (!ProductionCompanyRow) {
      throw new Error(`Production company '${company}' not found in the database.`);
    }
